...
function MatrixShader({
  ...
  // Create shader material once — shader source and initial uniforms never change;
  // runtime uniform mutations are handled by useFrame and the prop-sync effect below.
  ...
  // Dispose shaderMaterial on unmount to prevent GPU resource leaks
  useEffect(() => {
    return () => {
  ...
  // Sync animation-toggle props to existing uniforms so prop changes take effect
  // without requiring a full shader recompile.
  useEffect(() => {
    shaderMaterial.uniforms.u_enableWaveAnimation.value = enableWaveAnimation ? 1.0 : 0.0;
    shaderMaterial.uniforms.u_enableMouseHoverAnimation.value = enableMouseHoverAnimation ? 1.0 : 0.0;
    shaderMaterial.uniforms.u_enableCardBorderAnimation.value = enableCardBorderAnimation ? 1.0 : 0.0;
  }, [shaderMaterial, enableWaveAnimation, enableMouseHoverAnimation, enableCardBorderAnimation]);
